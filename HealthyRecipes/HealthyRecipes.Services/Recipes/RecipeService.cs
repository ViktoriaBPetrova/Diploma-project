...
namespace HealthyRecipes.Services.Recipes
{
    public class RecipeService: IRecipe
    {
        ...
        {
            var existing = await _context.Recipes.FirstOrDefaultAsync(r => r.Id == recipe.Id && !r.Deleted);
            if (existing == null)
                return false;
            ...
            //_context.Recipes.Update(recipe); OR
        ...
        public async Task<bool> SoftDeleteRecipeAsync(Guid id)
        {
            ...
            var existing = await _context.Recipes.FirstOrDefaultAsync(r => r.Id == id && !r.Deleted);
            if (existing == null)
                return false;
